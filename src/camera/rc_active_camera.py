...
class RCActiveCamera(PTZCamera):
    ...
    def processMessage(self, type, message):
	    ...
	    logging.debug("Received VP_CAM_CUSTOM_PAN message from conn:%s" % conn_id)
            ...
            logging.debug("Received VP_CAM_DEFAULT message from conn:%s" % conn_id)
    ...
    def isWaiting(self):
        """
        Returns True if the camera is waiting for a message from the VP.
...
def buildCamera(config, type):
    ...
    camera_module = RCActiveCamera(id, type, pos, up, dir)
